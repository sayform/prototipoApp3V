	...
	,propertyGroups: [
		{
			...
			properties: [
				...
				[
					{
						caption: 'Reset to original size'
						,name: 'resetdimensions'
						,type: 'link'
						,onClick: function(item){
						item.autoResize = true;
						if (item.imgSrc.assetType == "icon") {
							item.width = 100;
							item.height = 100;
							...
							// img not yet loaded
							if ($('#' + item.id + " img").prop('naturalWidth') == 0) {
								...
								img.onload = function (e) {
									...
									prx.itemBrowser.refresh();
									prx.states.refreshTimeline();
									prx.history.reset();
									prx.history.append();
									document.body.removeChild(img);
								};
								...
								img.style.visibility = 'hidden';
								document.body.appendChild(img);
								img.setAttribute('src', $('#' + item.id + " img").attr('src'));
								img.setAttribute('alt', 'na');

								return item;
							}

							item.width = $('#' + item.id + " img").prop('naturalWidth');
							item.height = $('#' + item.id + " img").prop('naturalHeight');
						}
